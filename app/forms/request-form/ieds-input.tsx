...
export const IedArrayInput = ({
	...
	const iedsTerceiros = allIeds
        ...
        function copyIed(index: number){
            append({
                name: control._formValues.entradas[nestIndex].ieds[index].name,
				manufacturer:control._formValues.entradas[nestIndex].ieds[index].manufacturer,
				address: control._formValues.entradas[nestIndex].ieds.length + 1,
				modules: control._formValues.entradas[nestIndex].ieds[index].modules,
			...
			{fields.map((item, k) => (
				...
				>
								...
								<div className="w-full flex justify-between bg-zinc-200 border-b-2 shadow-2xs">
									<Select
										onValueChange={(value) => {
											...
											if (selectedIed) {
												setValue(
													`entradas.${nestIndex}.ieds.${k}.manufacturer`,
                                    ...
                                    <Button onClick={()=>{copyIed(k)}} type="button" variant="outline" className="bg-radial from-zinc-200 to-zinc-300">
                                        <Copy />
                                    </Button>         
									<Button
										type="button"
					...
					<div className="flex items-center justify-center text-sm text-zinc-500 bg-zinc-100 mb-4 border-b rounded-b-md">
						{control._formValues.entradas[nestIndex].ieds[k].manufacturer
							? control._formValues.entradas[nestIndex].ieds[k].manufacturer
							: "Fabricante"}
					</div>
					...
					<section className="space-y-2">
						<div className="flex items-center gap-2 w-full">
							...
							{(control._formValues.entradas[nestIndex].ieds[k].name === "BM" ||
								control._formValues.entradas[nestIndex].ieds[k].name ===
									"COMM4" ||
								control._formValues.entradas[nestIndex].ieds[k].name ===
											...
											<InputGroupAddon align="inline-end">
												<Tooltip>
													...
													<TooltipContent className="max-w-40 text-justify">
														...
														conectados ou quantiade de Entradas Digitais para
														serem mapeadas
													</TooltipContent>
												</Tooltip>
											</InputGroupAddon>
