...
export const IedArrayOutput = ({ nestIndex, control, setValue, availableIeds, defaultIeds }: { nestIndex: number, control: any, setValue: any, availableIeds?: {nome: string, fabricante: string}[], defaultIeds?: IED[] }) => {
    ...
    const allIeds = availableIeds ? availableIeds.filter((item, index, self) =>
        index === self.findIndex((t) => (
            t.nome === item.nome && t.fabricante === item.fabricante
        ))
    ...
    return (
        <div className="p-1 min-h-[255px]">
            ...
            {fields.map((item, k) => (
                <motion.div key={k} className="flex flex-col mt-2  border rounded-md" initial={bouncingUpAnimation.initial} whileInView={bouncingUpAnimation.whileInView} transition={{ ...bouncingUpAnimation.transition, delay: parseFloat(`${k * 0.1} `) }}>
                                ...
                                <div className="w-full flex justify-between bg-zinc-200 border-b-2 shadow-2xs">
                                    <Select
                                        onValueChange={(value) => {
                                            ...
                                            if (selectedIed) {
                                                setValue(`saidas.${nestIndex}.ieds.${k}.manufacturer`, selectedIed.fabricante);
                                            }
                                        }}
                                        value={field.value}
                                    >
                                        ...
                                        <SelectContent>
                                            <SelectGroup>
                                                ...
                                                {iedsTerceiros.map((ied, i) => (
                                                    <SelectItem key={`treetech-${ied.nome}-${i}`} value={ied.nome}>{ied.nome} - {ied.fabricante}</SelectItem>
                                                ))}
                                            </SelectGroup>
                                        </SelectContent>
                                    </Select>
                    ...
                    <div className="flex items-center justify-center text-sm text-zinc-500 bg-zinc-100 mb-4 border-b rounded-b-md">
                        {control._formValues.saidas[nestIndex].ieds[k].manufacturer ? control._formValues.saidas[nestIndex].ieds[k].manufacturer : "Fabricante"}
                    </div>
                    ...
                    <section className="space-y-2">
                        <div className="flex items-center gap-2 w-full">
                            ...
                            {(control._formValues.saidas[nestIndex].ieds[k].name === "BM" ||
                                control._formValues.saidas[nestIndex].ieds[k].name === "COMM4" ||
                                control._formValues.saidas[nestIndex].ieds[k].name === "Entrada Digital do gateway"
                            ) && (
                                    <Controller
                                                ...
                                                <InputGroupAddon align="inline-end">
                                                    <Tooltip>
                                                        ...
                                                        <TooltipContent className="max-w-40 text-justify">
                                                            Quantidade de módulos do IED, ou quantidade de SPSs conectados ou quantiade de saidas Digitais para serem mapeadas
                                                        </TooltipContent>
                                                    </Tooltip>
                                                </InputGroupAddon>
                                            </InputGroup>}
            ...
            <motion.div animate={{ x: 0, opacity: 1 }} initial={{ x: 30, opacity: 0 }} transition={{}} className="mt-2 w-full">
                <Button
                    ...
                    variant="outline"
                    className="w-full"
                    onClick={() => append({ name: "", manufacturer: "", address: control._formValues.saidas[nestIndex].ieds.length + 1, modules: "", optional: "" })}
                >
